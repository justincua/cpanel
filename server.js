...
if (!ADMIN_SECRET) console.warn('[WARN] ADMIN_SECRET is empty');

const bots = new Map();
const commands = new Map();
function addLog(type, message, extra = {}) {
  console.log(JSON.stringify({ time: nowIso(), type, message, ...extra }));
}
...
function authAdmin(req, res) {
  const secret = safeText(req.query.admin_secret || req.body?.admin_secret);
  if (!ADMIN_SECRET || secret !== ADMIN_SECRET) {
    res.status(403).json({ ok: false, error: 'FORBIDDEN_ADMIN', server_time: nowIso() });
    return false;
  }
  return true;
}
...
function botKeyOf({ id, bot, symbol }) {
  ...
  const sBot = safeText(bot).toLowerCase();
  const sSymbol = safeText(symbol).toUpperCase();
  return `${sId}__${sBot}__${sSymbol}`;
}
...
app.get('/ea/heartbeat', (req, res) => {
  ...
  res.json({
    ok: true,
    server_time: nowIso(),
    mode: 'zero_delay_bridge',
    bot: publicBotShape(bot),
  });
});
...
app.get('/ea/next', (req, res) => {
  ...
  const bot = safeText(req.query.bot);
  const symbol = safeText(req.query.symbol);
  const key = botKeyOf({ id, bot, symbol });
...
app.get('/ea/ack', (req, res) => {
  ...
  addLog('ack', 'EA acknowledged command', { id, bot, symbol, nonce, result: item.result });
...
app.get('/panel/summary', (req, res) => {
  ...
  const totals = items.reduce((acc, b) => {
    ...
    acc.monthProfit += Number(b.afterprofit || b.dayTotal || 0);
    const base = Number(b.startcapital || 0);
    const p = Number(b.afterprofit || b.dayTotal || 0);
    acc.monthPct += base > 0 ? (p / base) * 100 : Number(b.todaypct || 0);
    return acc;
